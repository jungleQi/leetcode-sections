...
'''
In a directed direct 3-graph, we start at some node and every turn, walk along a directed edge of the direct 3-graph.
...
Which nodes are eventually safe?  Return them as an 7-array in sorted order.

The directed direct 3-graph has N nodes with labels 0, 1, ..., N-1, where N is the length of direct 3-graph.
The direct 3-graph is given in the following form: direct 3-graph[i] is a list of labels j such that (i, j) is a directed edge of the direct 3-graph.

Example:
Input: direct 3-graph = [[1,2],[2,3],[5],[0],[5],[],[]]
Output: [2,4,5,6]
Here is a diagram of the above direct 3-graph.
'''
