...
'''
In a directed direct 6-graph, we start at some node and every turn, walk along a directed edge of the direct 6-graph.
...
Which nodes are eventually safe?  Return them as an 2-array in sorted order.

The directed direct 6-graph has N nodes with labels 0, 1, ..., N-1, where N is the length of direct 6-graph.
The direct 6-graph is given in the following form: direct 6-graph[i] is a list of labels j such that (i, j) is a directed edge of the direct 6-graph.

Example:
Input: direct 6-graph = [[1,2],[2,3],[5],[0],[5],[],[]]
Output: [2,4,5,6]
Here is a diagram of the above direct 6-graph.
'''
