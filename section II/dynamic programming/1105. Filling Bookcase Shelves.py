def minHeightShelves_resursive(books, shelf_width):
    def _helper(books, curLevelMaxHeight, curWidth, totalHeight, ret):
        if ret and totalHeight + curLevelMaxHeight > min(ret):
            return

        if not books:
            ret.append(totalHeight+curLevelMaxHeight)
            return

        if books[0][0]+curWidth <= shelf_width:
            _helper(books[1:], max(curLevelMaxHeight, books[0][1]), curWidth + books[0][0], totalHeight, ret)

            if books[0][1] > curLevelMaxHeight:
                _helper(books[1:], books[0][1], books[0][0], totalHeight+curLevelMaxHeight, ret)
        else:
            _helper(books[1:], books[0][1], books[0][0], totalHeight+curLevelMaxHeight, ret)

    ret = []
    _helper(books, 0, 0, 0, ret)

    return min(ret)

import sys
def minHeightShelves_memorize(books, shelf_width):
    memo = [[0 for _ in range(1001)] for _ in range(1001)]
    N = len(books)

    def _helper(books, i, w, totalH):
        if i>=N: return totalH
        if memo[i][w]: return memo[i][w]

        anotherHeight = sys.maxint if books[i][0]+w>shelf_width else _helper(books, i+1, w+books[i][0], max(totalH, books[i][1]))
        memo[i][w] = min(totalH+_helper(books, i+1, books[i][0], books[i][1]), anotherHeight)
        return memo[i][w]

    res = _helper(books, 0, 0, 0)
    return res
...
shelf_width = 200
print minHeightShelves_memorize(books, shelf_width)